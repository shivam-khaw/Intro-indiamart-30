  ...
  <div class="col-12">
    <div class="card">
      <div class="card-header">
        <div class="row">
          <div class="col-8">
            <h4>Deal fields Mapping</h4>
          </div>
          <div class="col-4">
            @foreach($result as $val)
            @if($val->status == 1)
            <form action="{{ route('statusUpdate', ['id' => $val->id, 'status' => 0]) }}" method="POST" style="display: inline;">
              @csrf
              @method('post')
              <button type="submit" class="btn btn-success">ON</button>
            </form>
            @elseif($val->status == 0)
            <form action="{{ route('statusUpdate', ['id' => $val->id, 'status' => 1]) }}" method="POST" style="display: inline;">
              @csrf
              @method('post')
              <button type="submit" class="btn btn-danger">OFF</button>
            </form>
            @endif
            <p>From the button above you can connect deal or not connect</p>
            @endforeach
      ...
      <div class="card-body">
        <form action="{{url('add_fields_deal')}}" method="post">
          @csrf
          <div class="row">
            <div class="col-4">
